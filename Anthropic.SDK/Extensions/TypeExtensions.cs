    ...
    internal static class TypeExtensions
    {
        ...
        public static JsonObject GenerateJsonSchema(this Type type, JsonObject rootSchema)
        {
            ...
            else if (type.IsEnum)
            {
                ...
                {
                    schema["enum"].AsArray().Add(JsonNode.Parse(JsonSerializer.Serialize(value, AnthropicClient.JsonSerializationOptions)));
                }
            }
            ...
            else
            {
                ...
                foreach (var member in members)
                {
                    ...
                    if (functionPropertyAttribute != null)
                    {
                        ...
                        {
                            defaultValue = JsonNode.Parse(JsonSerializer.Serialize(functionPropertyAttribute.DefaultValue, AnthropicClient.JsonSerializationOptions));
                        ...
                        if (functionPropertyAttribute.PossibleValues is { Length: > 0 })
                        {
                            ...
                            {
                                var @enum = JsonNode.Parse(JsonSerializer.Serialize(value, AnthropicClient.JsonSerializationOptions));
                            ...
                            {
                                enums.Add(JsonNode.Parse(defaultValue.ToJsonString(AnthropicClient.JsonSerializationOptions)));
                            }
